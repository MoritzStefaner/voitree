...
public class WB_Circle {
	 ...
	 */
	public void setCenter(final WB_XY center) {
		_center = center;
	}
	 ...
	 * @param r radius
	 */
	public WB_Circle(final WB_XY c, final double r) {
		_center = c;
		_radius = r;
	}
	...
	public WB_Circle(final double x, final double y, final double r) {
		_center = new WB_XY(x, y);
		_radius = r;
	}

	public static WB_Circle getCircleThrough3Points(final WB_XY p0,
			final WB_XY p1, final WB_XY p2) {
	...
	public static WB_Circle getCircleTangentTo3Lines(final WB_Line2D L0,
		...
		final WB_XY p0 = WB_ClosestPoint2D.closestPoint2D(L0, L1).p1;
		final WB_XY p1 = WB_ClosestPoint2D.closestPoint2D(L1, L2).p1;
		final WB_XY p2 = WB_ClosestPoint2D.closestPoint2D(L0, L2).p1;

		final WB_Triangle2D T = new WB_Triangle2D(p0, p1, p2);
		return T.incircle();
	}

	public static ArrayList<WB_Circle> getCircleThrough2Points(final WB_XY p0,
			final WB_XY p1, final double r) {
		...
		final ArrayList<WB_XY> intersection = WB_Intersection2D.intersect2D(C0,
				C1);
	...
	public static ArrayList<WB_Circle> getCircleTangentToLineThroughPoint(
			final WB_Line2D L, final WB_XY p, final double r) {
		final ArrayList<WB_Circle> result = new ArrayList<WB_Circle>();
		...
		if (WB_Epsilon.isZero(tmp2)) {
			...
			tmp2 = Math.sqrt(tmp2);
			final WB_XY tmpp = new WB_XY(p.x - a * tmp1, p.y - b * tmp1);
	...
	public static ArrayList<WB_Circle> getCircleTangentTo2Lines(
		...
		double cy = +(L1.a() * (L0.c() + discrm0) - L0.a() * (L1.c() + discrm1))
				* invDenom;
		result.add(new WB_Circle(new WB_XY(cx, cy), r));
	...
	public static ArrayList<WB_Circle> getCircleThroughPointTangentToCircle(
			final WB_XY p, final WB_Circle C, final double r) {
		final ArrayList<WB_Circle> result = new ArrayList<WB_Circle>(4);
		...
		if (dcp > C.getRadius() + 2 * r) {
			...
			ArrayList<WB_XY> intersection = WB_Intersection2D.intersect2D(
					ctmp1, ctmp2);
			for (int i = 0; i < intersection.size(); i++) {
				result.add(new WB_Circle(intersection.get(i), r));
			}
	...
	public static ArrayList<WB_Circle> getCircleTangentToLineAndCircle(
		...
		final WB_Circle C2 = new WB_Circle(C.getCenter(), WB_Fast.abs(C.getRadius()
				- r));
		final ArrayList<WB_XY> intersections = new ArrayList<WB_XY>();
