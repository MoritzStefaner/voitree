...
public class WB_Line2D {
	 ...
	 * @param d direction
	 */
	public WB_Line2D(final WB_XY o, final WB_XY d) {
		origin = o.get();
		direction = d.get();
		direction.normalize();
	...
	public WB_Line2D(final double p1x, final double p1y, final double p2x,
			final double p2y) {
		origin = new WB_XY(p1x, p1y);
		direction = new WB_XY(p2x - p1x, p2y - p1y);
		direction.normalize();
	...
	public double getT(final WB_XY p) {
		double t = Double.NaN;
		final WB_XY proj = WB_ClosestPoint2D.closestPoint2D(p, this);
	...
	public WB_XY getNormal() {
		return new WB_XY(-direction.y, direction.x);
	}
	 ...
	 *
	  */
	public WB_ClassifyPointToLine2D classifyPointToLine2D(final WB_XY p) {
	 ...
	 *
	  */
	public static WB_ClassifyPointToLine2D classifyPointToLine2D(final WB_XY p,
			final WB_Line2D L) {
	...
	public WB_ClassifySegmentToLine2D classifySegmentToLine2D(
		...
		final WB_ClassifyPointToLine2D a = classifyPointToLine2D(seg.getOrigin());
	...
	public static WB_Line2D getLineTangentToCircleAtPoint(final WB_Circle C,
			final WB_XY p) {
		final WB_XY v = p.subAndCopy(C.getCenter());
		return new WB_Line2D(p, new WB_XY(-v.y, v.x));

	}

	public static ArrayList<WB_Line2D> getLinesTangentToCircleThroughPoint(
			final WB_Circle C, final WB_XY p) {
		...
		if (WB_Epsilon.isZero(dcp - C.getRadius())) {
			final WB_XY u = p.subAndCopy(C.getCenter());
			result.add(new WB_Line2D(p, new WB_XY(-u.y, u.x)));
			...
			result.add(new WB_Line2D(p, new WB_XY(-((r2 * u.y) + rad) / denom,
					(r2 - (num + u.y * rad) / denom) / u.x)));
			result.add(new WB_Line2D(p, new WB_XY(-((r2 * u.y) - rad) / denom,
					(r2 - (num - u.y * rad) / denom) / u.x)));
	...
	public static ArrayList<WB_Line2D> getLinesTangentTo2Circles(
		...
		final WB_XY w = C1.getCenter().subAndCopy(C0.getCenter());
		...
		if (!WB_Epsilon.isZero(rdiff)) {
			...
			WB_XY[] dir = getDirections(w, a);

			WB_XY org = new WB_XY(C0.getCenter().x + s * w.x, C0.getCenter().y + s
					* w.y);
			...
			org = new WB_XY(C0.getCenter().x + s * w.x, C0.getCenter().y + s * w.y);
			result.add(new WB_Line2D(org, dir[0]));
			result.add(new WB_Line2D(org, dir[1]));

		} else {
			final WB_XY mid = (C0.getCenter().addAndCopy(C1.getCenter())).mult(0.5);
			final double a = Math.sqrt(WB_Fast.abs(wlensqr - 4 * C0.getRadius()
					* C0.getRadius()));
			final WB_XY[] dir = getDirections(w, a);
			...
			w.x *= invwlen;
			w.y *= invwlen;
			result.add(new WB_Line2D(new WB_XY(mid.x + C0.getRadius() * w.y, mid.y
					- C0.getRadius() * w.x), w));
			result.add(new WB_Line2D(new WB_XY(mid.x - C0.getRadius() * w.y, mid.y
					+ C0.getRadius() * w.x), w));
	...
	private static WB_XY[] getDirections(final WB_XY w, final double a) {
		final WB_XY[] dir = new WB_XY[2];
		...
		if (WB_Fast.abs(w.x) >= WB_Fast.abs(w.y)) {
			...
			dir[0] = new WB_XY((a - w.y * dir0y) * invwx, dir0y);
			final double dir1y = -0.5 * (c1 - discr) * invc2;
			dir[1] = new WB_XY((a - w.y * dir1y) * invwx, dir1y);
			...
			dir[0] = new WB_XY(dir0x, (a - w.x * dir0x) * invwy);
			final double dir1x = -0.5 * (c1 - discr) * invc2;
			dir[1] = new WB_XY(dir1x, (a - w.x * dir1x) * invwy);
	...
	public static WB_Line2D getPerpendicularLineThroughPoint(final WB_Line2D L,
			final WB_XY p) {
		return new WB_Line2D(p, new WB_XY(-L.getDirection().y, L.getDirection().x));
	...
	public static WB_Line2D getBisector(final WB_XY p, final WB_XY q) {
		return new WB_Line2D(WB_XY.interpolate(p, q, 0.5), new WB_XY(p.y - q.y,
				q.x - p.x));
	}

	public static WB_Line2D[] getParallelLines(final WB_Line2D L, final double d) {
		final WB_Line2D[] result = new WB_Line2D[2];
		result[0] = new WB_Line2D(new WB_XY(L.getOrigin().x - d * L.getDirection().y,
				L.getOrigin().y + d * L.getDirection().x), L.getDirection());
		result[1] = new WB_Line2D(new WB_XY(L.getOrigin().x + d * L.getDirection().y,
				L.getOrigin().y - d * L.getDirection().x), L.getDirection());
		return result;
	}
	...
	public static WB_Line2D[] getPerpendicularLinesTangentToCircle(
		...
		result[0] = new WB_Line2D(
				new WB_XY(C.getCenter().x + C.getRadius() * L.getDirection().x, C
						.getCenter().y
						+ C.getRadius() * L.getDirection().y), new WB_XY(-L
						.getDirection().y, L.getDirection().x));
