...
public class WB_Voronoi2D {
	...
	private class VoronoiEdge2D {
		...
		VoronoiEdge2D() {
			endPoints = new WB_XY[2];
			betweenSites = new WB_XY[2];
			indices = new int[2];
		}
	}
	...
	private FastMap<WB_XY, Integer>			indices;

	public WB_Voronoi2D() {
	}

	public ArrayList<WB_IndexedBisector2D> generateVoronoi(final double minX,
			final double maxX, final double minY, final double maxY,
			final WB_XY[] points) {
		final ArrayList<WB_XY> ppoints = new ArrayList<WB_XY>();
		for (final WB_XY point : points) {
			ppoints.add(point);
		}
		return generateVoronoi(minX, maxX, minY, maxY, ppoints);
	}

	public ArrayList<WB_IndexedBisector2D> generateVoronoi(double minX,
			double maxX, double minY, double maxY, final ArrayList<WB_XY> points) {
		double temp = 0;
		if (minX > maxX) {
			temp = minX;
			minX = maxX;
		...
		final ArrayList<WB_IndexedBisector2D> result = new ArrayList<WB_IndexedBisector2D>(
				allEdges.size());
		result.addAll(allEdges);
		return result;
	}

	private void collectSites(final List<WB_XY> pointsIn) {
		allEdges = new FastList<WB_IndexedBisector2D>();

		final FastList<WB_XY> points = new FastList<WB_XY>();
		final WB_KDTree2D<Integer> kdtree = new WB_KDTree2D<Integer>();
		WB_KDNeighbor2D<Integer>[] neighbors;
		WB_XY p = new WB_XY(pointsIn.get(0));
		kdtree.put(p, 0);
		points.add(p);
		for (int i = 1; i < pointsIn.size(); i++) {
			p = new WB_XY(pointsIn.get(i));
	...
	private void sortSites(final FastList<WB_XY> points) {
		sites = new WB_XY[nsites];
		xmin = points.get(0).x;
		ymin = points.get(0).y;
		xmax = xmin;
		ymax = ymin;
		indices = new FastMap<WB_XY, Integer>();
		for (int i = 0; i < nsites; i++) {
			sites[i] = new WB_XY(points.get(i).x, points.get(i).y);
	...
	private boolean createVoronoi2D() {
		WB_XY newsite, bot, top, temp, p;
		WB_XY v;
		WB_XY newintstar = null;
		int pm;
	...
	private void insertHalfedgeInPointqueue(final VoronoiHalfedge2D he,
			final WB_XY v, final double offset) {
	...
	private WB_XY pointQueueMin() {
		final WB_XY result = new WB_XY();
	...
	private VoronoiHalfedge2D ELleftbnd(final WB_XY p) {
		int i, marker;
		VoronoiHalfedge2D he;
	...
	private VoronoiEdge2D bisect(final WB_XY s1, final WB_XY s2) {
	...
	private void makevertex(final WB_XY v) {
		nvertices += 1;
	}
	...
	private WB_XY leftreg(final VoronoiHalfedge2D he) {
	...
	private void clipToBoundary(final VoronoiEdge2D e) {
		...
		ib.start = new WB_XY(x1, y1);
		ib.end = new WB_XY(x2, y2);
		ib.i = e.indices[0];
		ib.j = e.indices[1];
		allEdges.add(ib);
	}

	private void endpoint(final VoronoiEdge2D e, final int lr, final WB_XY s) {
		e.endPoints[lr] = s;
	...
	private WB_XY rightreg(final VoronoiHalfedge2D he) {
		if (he.voronoiEdge == null) {
	...
	private WB_XY intersect(final VoronoiHalfedge2D el1,
