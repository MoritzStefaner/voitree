...
public class WB_Triangle2D {
	 ...
	 */
	public WB_Triangle2D(final WB_XY p1, final WB_XY p2, final WB_XY p3) {
		this.p1 = p1;
		this.p2 = p2;
		this.p3 = p3;
		update();
	...
	public WB_Circle incircle() {
		...
		if (!degenerate) {
			...
			result.setRadius(0.5 * Math.sqrt(((b + c - a) * (c + a - b) * (a
					+ b - c))
					/ abc));
			final WB_XY ta = p1.multAndCopy(a);
			final WB_XY tb = p2.multAndCopy(b);
			final WB_XY tc = p3.multAndCopy(c);
	 ...
	 * @return point
	 */
	public WB_XY getPointFromTrilinear(final double x, final double y,
			final double z) {
		if (!degenerate) {

			final double abc = a * x + b * y + c * z;
			final WB_XY ea = p2.subAndCopy(p3);
			final WB_XY eb = p1.subAndCopy(p3);
			ea.mult(b * y);
			eb.mult(a * x);
			ea.add(eb);
			ea.div(abc);
	 ...
	 * @return point
	 */
	public WB_XY getPointFromBarycentric(final double x, final double y,
			final double z) {
		if (!degenerate) {
	 ...
	 */
	public WB_XYZ getBarycentric(final WB_XY p) {
		...
		nu *= ood;
		nv *= ood;
		return new WB_XYZ(nu, nv, 1 - nu - nv);
	 ...
	 *
	 * @param p point
	 * @param result point to store barycentric coordinates
	 */
	public void getBarycentricInto(final WB_XY p, final WB_XYZ result) {
		final double m = (p3.x - p1.x) * (p2.y - p1.y) - (p3.y - p1.y)
				* (p2.x - p1.x);
		double ood;
		result.x = twiceSignedTriArea2D(p.x, p.y, p2.x, p2.y, p3.x, p3.y);
		result.y = twiceSignedTriArea2D(p.x, p.y, p3.x, p3.y, p1.x, p1.y);
		ood = -1.0 / m;
		result.x *= ood;
		result.y *= ood;
	 ...
	 * @return true, false
	 */
	public static boolean sameSide2D(final WB_XY p1, final WB_XY p2,
			final WB_XY A, final WB_XY B) {
		final WB_XY t1 = B.get().sub(A);
		final WB_XY t2 = p1.get().sub(A);
		final WB_XY t3 = p2.get().sub(A);
	...
	public static boolean pointInTriangle2D(final WB_XY p, final WB_Triangle2D T) {
	 ...
	 * @return true, false
	 */
	public static boolean pointInTriangleBary2D(final WB_XY p, final WB_XY A,
			final WB_XY B, final WB_XY C) {
		...
		// Compute vectors
		final WB_XY v0 = C.get().sub(A);
		final WB_XY v1 = B.get().sub(A);
		final WB_XY v2 = p.get().sub(A);
	...
	public static boolean pointInTriangleBary2D(final WB_XY p,
	 ...
	 * @return the double
	 */
	public static double twiceSignedTriArea2D(final WB_XY p1, final WB_XY p2,
			final WB_XY p3) {
	...
	public boolean isCounterClockwise() {
		final WB_M33 m33 = new WB_M33(1, 1, 1, p1.x, p2.x, p3.x, p1.y, p2.y,
				p3.y);
		return m33.det() > -WB_Epsilon.EPSILON;
