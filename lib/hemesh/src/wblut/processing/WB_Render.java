...
import wblut.frame.WB_Node;
import wblut.frame.WB_Strut;
...
import wblut.geom2D.WB_Segment2D;
import wblut.geom2D.WB_Triangle2D;
...
public class WB_Render {
	...
	public void drawFaceNormal(final HE_Face f, final double d) {
		...
		final WB_Point p2 = new WB_Point(f.getFaceNormal().multAndCopy(d))
				.add(p1);
	...
	*/
	...
	public void drawPoint(final WB_XYZ v, final double d) {
		_home.pushMatrix();
		_home.translate((float) (v.x), (float) (v.y), (float) (v.z));
		_home.box((float) d);
		_home.popMatrix();
	...
	public void drawPoint(final Collection<? extends WB_XYZ> points,
			final double d) {
		for (final WB_XYZ v : points) {
			drawPoint(v, d);
		}
	}
	...
	public void drawHalfedge(final HE_Halfedge he, final double d,
			final double s, final HE_Mesh mesh) {
	...
	public void drawHalfedge(final HE_Halfedge he, final double d,
			final double s, final double f, final HE_Mesh mesh) {
	...
	private void drawConcaveFace(final HE_Face f) {
		...
		final ArrayList<WB_IndexedTriangle2D> tris = f.triangulate();
		final ArrayList<HE_Vertex> vertices = f.getFaceVertices();
		WB_Point v0, v1, v2;
	 ...
	 */
	private void drawConvexShapeFromVertices(
			final ArrayList<HE_Vertex> vertices, final boolean smooth,
			final HE_Mesh mesh) {
		...
		if (degree < 3) {
			...
			if (smooth) {
				...
				final WB_Normal n0 = v0.getVertexNormal();
				final HE_Vertex v1 = vertices.get(1);
				final WB_Normal n1 = v1.getVertexNormal();
				final HE_Vertex v2 = vertices.get(2);
				final WB_Normal n2 = v2.getVertexNormal();
			...
			if (smooth) {
				...
				final WB_Normal n1 = v1.getVertexNormal();
				final WB_Normal n2 = v2.getVertexNormal();
				final WB_Normal n3 = v3.getVertexNormal();
			...
			if (smooth) {
				...
				final WB_Normal n2 = v2.getVertexNormal();
				final WB_Normal n3 = v3.getVertexNormal();
				final WB_Normal n4 = v4.getVertexNormal();
	...
	private void drawConvexShapeFromVertices(
			final ArrayList<HE_Vertex> vertices, final boolean smooth,
			final HE_Mesh mesh, final PGraphics pg) {
		final int degree = vertices.size();
		if (degree < 3) {
			// yeah, right...
		} else if (degree == 3) {
	...
	public void drawPolygonEdges(final Collection<WB_Polygon> polygons) {
	...
	public void drawPolygon(final Collection<WB_Polygon> polygons) {
		final Iterator<WB_Polygon> polyItr = polygons.iterator();
		while (polyItr.hasNext()) {
	...
	public void draw(final WB_Frame frame) {
		final ArrayList<WB_Strut> struts = frame.getStruts();
	...
	public void drawNodes(final WB_Frame frame, final double s) {
		final ArrayList<WB_Node> nodes = frame.getNodes();
		for (int i = 0; i < frame.getNumberOfNodes(); i++) {
			draw(nodes.get(i), s);
		}
	}

	public void draw(final WB_Strut strut) {
		_home.line(strut.start().xf(), strut.start().yf(), strut.start().zf(),
				strut.end().xf(), strut.end().yf(), strut.end().zf());
	}

	public void draw(final WB_Node node, final double s) {
		_home.pushMatrix();
	...
	*/
	...
	public void draw(final WB_XYZ point, final double d) {
		_home.pushMatrix();
	...
	public void draw(final Collection<? extends WB_XYZ> points, final double d) {
