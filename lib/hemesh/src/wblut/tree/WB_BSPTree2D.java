...
import wblut.geom2D.WB_ClassifyPointToLine2D;
import wblut.geom2D.WB_ClassifyPolygonToLine2D;
import wblut.geom2D.WB_ClassifySegmentToLine2D;
import wblut.geom2D.WB_Distance2D;
import wblut.geom2D.WB_ExplicitSegment2D;
import wblut.geom2D.WB_Intersection2D;
...
public class WB_BSPTree2D {
	...
	public int pointLocation(final WB_XY p) {
		return pointLocation(root, p);

	}

	public int pointLocation(final double x, final double y) {
		return pointLocation(root, new WB_XY(x, y));

	}

	private int pointLocation(final WB_BSPNode2D node, final WB_XY p) {
		...
		for (int i = 0; i < node.segments.size(); i++) {
			...
			for (int j = 0; j < partSegments.size(); j++) {
				...
				if (u0 < u1) {
					...
					if (intersection[0] == 2) {
						final WB_XY pi = L.getPoint(intersection[1]);
						final WB_XY pj = L.getPoint(intersection[2]);
