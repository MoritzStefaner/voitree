...
public class WB_Sphere {
	 ...
	 * @param r radius
	 */
	public WB_Sphere(final WB_Point c, final double r) {
		this.c = c;
		this.r = r;
	}
	 ...
	 */
	public WB_Sphere(final WB_Point c, final double r, final boolean copy) {
		if (copy) {
			this.c = c.get();
			this.r = r;
		} else {
	...
	public void setCenter(final WB_Point c) {
		this.c = c;
	}
	 ...
	 * @return sphere
	 */
	public static WB_Sphere sphereFromDistantPoints(final WB_Point[] points,
			final int numPoints) {
		int minx = 0;
		int maxx = 0;
		int miny = 0;
		...
		final WB_Point c = (points[min].addAndCopy(points[max])).mult(0.5);
	...
	public void growSpherebyPoint(final WB_Point p) {
		final WB_Vector d = p.subToVector(c);
	 ...
	 */
	public WB_XYZ projectToSphere(final WB_XYZ v) {
		final WB_XYZ vc = v.subAndCopy(c);
