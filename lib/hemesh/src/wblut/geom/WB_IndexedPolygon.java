...
public class WB_IndexedPolygon implements WB_Polygon {
	...
	public ArrayList<WB_IndexedTriangle> triangulate() {
		...
		if (tmp.n > 3) {
			...
			if (tmp.n == 3) {
				tris.add(new WB_IndexedTriangle(0, 1, 2, tmp3d.allpoints));
			}
		} else if (tmp.n == 3) {
			tris.add(new WB_IndexedTriangle(0, 1, 2, allpoints));
	...
	public static ArrayList<WB_IndexedPolygon> extractPolygons(
			final ArrayList<WB_IndexedSegment> segs, final WB_Point[] points) {
		final ArrayList<WB_IndexedPolygon> result = new ArrayList<WB_IndexedPolygon>();
		final ArrayList<WB_IndexedSegment> leftovers = new ArrayList<WB_IndexedSegment>();
		final ArrayList<WB_IndexedSegment> cleanedsegs = clean(segs, points);
		leftovers.addAll(cleanedsegs);
		while (leftovers.size() > 0) {
			final ArrayList<WB_IndexedSegment> currentPolygon = new ArrayList<WB_IndexedSegment>();
	...
	public static ArrayList<WB_IndexedSegment> clean(
			final List<WB_IndexedSegment> segs, final WB_Point[] points) {
		final ArrayList<WB_IndexedSegment> cleanedsegs = new ArrayList<WB_IndexedSegment>();
	...
	private static boolean tryToFindLoop(
			final ArrayList<WB_IndexedSegment> segs,
			final ArrayList<WB_IndexedSegment> loop, final WB_Point[] points) {
		final ArrayList<WB_IndexedSegment> localSegs = new ArrayList<WB_IndexedSegment>();
	...
	public WB_Polygon2D toPolygon2D() {
		...
		for (int i = 0; i < n; i++) {
			lpoints[i] = P.embedPoint(getPoint(i));
		}
