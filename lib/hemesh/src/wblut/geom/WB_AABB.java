...
public class WB_AABB {
	...
	protected WB_Point	min;

	/** Maximum x, y and z values. */
	protected WB_Point	max;
	 ...
	 */
	public WB_AABB(final WB_Point[] points, final int n) {
		min = new WB_Point(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY,
				Double.POSITIVE_INFINITY);
		max = new WB_Point(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY,
	...
	public WB_AABB(final double[] min, final double[] max) {
		this.min = new WB_Point(min[0], min[1], min[2]);
		this.max = new WB_Point(max[0], max[1], max[2]);
		check();
	}
	 ...
	 * @param max maximum values as WB_Point
	 */
	public void set(final WB_Point min, final WB_Point max) {
		this.min = min.get();
		this.max = max.get();
		check();
	}
	...
	public WB_Point getCenter() {
		return new WB_Point(0.5 * (max.x + min.x), 0.5 * (max.y + min.y),
				0.5 * (max.z + min.z));

	}

	public WB_AABB union(final WB_AABB aabb) {
		final WB_Point newmin = new WB_Point(Math.min(min.x, aabb.getMin().x),
				Math.min(min.y, aabb.getMin().y), Math.min(min.z,
						aabb.getMin().z));
		final WB_Point newmax = new WB_Point(Math.max(max.x, aabb.getMax().x),
	 ...
	 */
	public double sqDistance(final WB_XYZ p) {
	 ...
	 * @return distance
	 */
	public double distance(final WB_XYZ p) {
		return WB_Distance.sqDistance(p, this);
	}
