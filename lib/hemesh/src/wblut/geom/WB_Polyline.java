...
public class WB_Polyline {
	 ...
	 */
	public WB_Polyline(final WB_Point[] points, final int n) {
		this.points = points;
		this.n = n;
	}
	 ...
	 */
	public WB_Polyline(final WB_Point[] points, final int n, final boolean copy) {
		if (copy == false) {
			this.points = points;
		} else {
	 ...
	 */
	public WB_Polyline(final ArrayList<WB_Point> points) {
		n = points.size();
		this.points = new WB_Point[n];
		for (int i = 0; i < n; i++) {
	 ...
	 */
	public WB_Polyline addPoint(final int i, final WB_Point p) {
		final WB_Point[] newPoints = new WB_Point[n + 1];
		for (int j = 0; j < i; j++) {
			newPoints[j] = points[j];
		}
	...
	public WB_Polyline smooth() {
		final WB_Point[] newPoints = new WB_Point[2 * n - 1];
		...
		final WB_Point[] sPoints = new WB_Point[2 * n - 1];
