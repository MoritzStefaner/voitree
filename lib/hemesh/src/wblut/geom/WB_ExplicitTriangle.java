...
public class WB_ExplicitTriangle implements WB_Triangle {
	 ...
	 */
	public WB_ExplicitTriangle(final WB_Point p1, final WB_Point p2,
			final WB_Point p3) {
		this.p1 = p1;
		this.p2 = p2;
		this.p3 = p3;
	 ...
	 */
	public WB_ExplicitTriangle(final WB_Point p1, final WB_Point p2,
			final WB_Point p3, final boolean copy) {
		if (!copy) {
			this.p1 = p1;
			this.p2 = p2;
			this.p3 = p3;
	...
	public WB_Point getPointFromTrilinear(final double x, final double y,
		...
		if (!degenerate) {
			...
			final WB_Point ea = p2.subAndCopy(p3);
			final WB_Point eb = p1.subAndCopy(p3);
			ea.mult(b * y);
			eb.mult(a * x);
			ea.add(eb);
			ea.div(abc);
	...
	public WB_Point getBarycentric(final WB_Point p) {
		final WB_Vector m = WB_Vector.cross(p3.subToVector(p1),
				p2.subToVector(p1));
		double nu, nv, ood;
		...
		if (x >= y && x >= z) {
			nu = WB_Triangle2D.twiceSignedTriArea2D(p.y, p.z, p2.y, p2.z, p3.y,
					p3.z);
			nv = WB_Triangle2D.twiceSignedTriArea2D(p.y, p.z, p3.y, p3.z, p1.y,
					p1.z);
			ood = 1.0 / m.x;
		} else if (y >= x && y >= z) {
			nu = WB_Triangle2D.twiceSignedTriArea2D(p.x, p.z, p2.x, p2.z, p3.x,
					p3.z);
			nv = WB_Triangle2D.twiceSignedTriArea2D(p.x, p.z, p3.x, p3.z, p1.x,
					p1.z);
			ood = -1.0 / m.y;
		} else {
			nu = WB_Triangle2D.twiceSignedTriArea2D(p.x, p.y, p2.x, p2.y, p3.x,
					p3.y);
			nv = WB_Triangle2D.twiceSignedTriArea2D(p.x, p.y, p3.x, p3.y, p1.x,
					p1.y);
			ood = -1.0 / m.z;
		}
		nu *= ood;
