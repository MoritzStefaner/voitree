...
public class WB_IndexedTriangle implements WB_Triangle {
	...
	public WB_Point getPointFromTrilinear(final double x, final double y, final double z) {
		if (!degenerate) {
			...
			final WB_Point ea = points[i2].subAndCopy(points[i3]);
			final WB_Point eb = points[i1].subAndCopy(points[i3]);
			ea.mult(b * y);
			eb.mult(a * x);
			ea.add(eb);
			ea.div(abc);
	...
	public WB_Point getBarycentric(final WB_Point p) {
		final WB_Vector m = WB_Vector.cross(points[i3].subToVector(points[i1]),
		...
		if (x >= y && x >= z) {
			nu = WB_Triangle2D.twiceSignedTriArea2D(p.y, p.z, points[i2].y,
					points[i2].z, points[i3].y, points[i3].z);
			nv = WB_Triangle2D.twiceSignedTriArea2D(p.y, p.z, points[i3].y,
					points[i3].z, points[i1].y, points[i1].z);
			ood = 1.0 / m.x;
		} else if (y >= x && y >= z) {
			nu = WB_Triangle2D.twiceSignedTriArea2D(p.x, p.z, points[i2].x,
					points[i2].z, points[i3].x, points[i3].z);
			nv = WB_Triangle2D.twiceSignedTriArea2D(p.x, p.z, points[i3].x,
					points[i3].z, points[i1].x, points[i1].z);
			ood = -1.0 / m.y;
		} else {
			nu = WB_Triangle2D.twiceSignedTriArea2D(p.x, p.y, points[i2].x,
					points[i2].y, points[i3].x, points[i3].y);
			nv = WB_Triangle2D.twiceSignedTriArea2D(p.x, p.y, points[i3].x,
