...
public class HES_Planar extends HES_Subdividor {
	...
	public HE_Mesh apply(final HE_Mesh mesh) {
		...
		orig.addVertices(mesh.getVerticesAsArrayList());
		...
		while (fItr.hasNext()) {
			...
			if ((face.getFaceOrder() == 6) && keepTriangles) {
				...
				HE_Mesh.cycleHalfedges(faceHalfedges,
						wblut.hemesh.options.HE_CycleOption.NORMAL);
			} else {
				HE_Halfedge origHE1 = startHE;
				do {
					final HE_Face newFace = new HE_Face();
	...
	public HE_Mesh apply(final HE_Selection selection) {
		...
		while (fItr.hasNext()) {
			...
			if ((face.getFaceOrder() == 6) && keepTriangles) {
				...
				HE_Mesh.cycleHalfedges(faceHalfedges, HE_CycleOption.NORMAL);
			} else {
				HE_Halfedge origHE1 = startHE;
				do {
					final HE_Face newFace = new HE_Face();
		...
		selection.parent.pairHalfedgesSilent();
