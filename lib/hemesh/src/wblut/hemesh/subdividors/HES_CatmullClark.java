 ...
 */
package wblut.hemesh.subdividors;
...
public class HES_CatmullClark extends HES_Subdividor {
	...
	public HE_Mesh apply(final HE_Mesh mesh) {
		...
		final HashMap<Integer, WB_Point> avgFC = new HashMap<Integer, WB_Point>();
		HE_Vertex v;
		...
		mesh.quadSplitFaces();
		final HashMap<Integer, WB_Point> newPositions = new HashMap<Integer, WB_Point>();
		final HE_Selection all = mesh.selectAllFaces();
		final ArrayList<HE_Vertex> boundary = all.getOuterVertices();
		final ArrayList<HE_Vertex> inner = all.getInnerVertices();

		HE_Vertex n;
		ArrayList<HE_Vertex> neighbors;
		vItr = inner.iterator();
		while (vItr.hasNext()) {
			v = vItr.next();
			if (v.getLabel() == -1) {
				...
				p.div(order);
				newPositions.put(v.key(), WB_Point.interpolate(v, p,
		...
		while (vItr.hasNext()) {
			...
			if (keepBoundary) {
				...
				newPositions.put(v.key(), (nc > 1) ? WB_Point.interpolate(v, p
						.scale(1.0 / c), blendFactor.value(v.x, v.y, v.z)) : v);
			}
		}
	...
	public HE_Mesh apply(final HE_Selection selection) {
		...
		final ArrayList<HE_Vertex> boundary = selection.getBoundaryVertices();
		final ArrayList<HE_Vertex> outer = selection.getOuterVertices();
		final ArrayList<HE_Vertex> inner = selection.getInnerVertices();
		ArrayList<HE_Face> sharedFaces;
		...
		while (vItr.hasNext()) {
			...
			if (keepEdges) {
				...
				if (nc > 1) {
					p.scale(1.0 / c);
					if (planes.size() == 1) {
						p = WB_ClosestPoint.closestPoint(p, planes.get(0));
					}/*
