...
public class HEC_FromFrame extends HEC_Creator {
	...
	class StrutNodeConnection {
		...
		StrutNodeConnection(final WB_Node node, final WB_Strut strut,
	...
	private void getNodeTypes() {
		int i = 0;
		double minSpan;
		for (final WB_Node node : frame.getNodes()) {
			if (node.getOrder() == 0) {
	...
	private void createNodeStrutConnection(final WB_Node node, final int i,
			final double maxoff, final double off, final double rad) {
		final WB_Strut strut = node.getStrut(i);
	...
	private void createVertices() {
		...
		for (int id = 0; id < frame.getNumberOfStruts() * 2; id++) {
			...
			for (int j = 0; j < strutFacets; j++) {
				...
				p.add(v, so);
				final WB_Vector localu = u.multAndCopy(sr);
	...
	private int getStrutIndex(final WB_Node node, final WB_Strut strut) {
		if (node == strut.start()) {
	...
	private void createStruts() {
		int i = 0;
		for (final WB_Strut strut : frame.getStruts()) {
			...
			if (nodeTypes[i] == NodeType.ENDPOINT) {
				...
				if ((nodeTypes[i] != NodeType.ISOLATED) || createIsolatedNodes) {
					...
					if ((nodeTypes[i] != NodeType.STRAIGHT)
						...
						hullPoints.addAll(ball.getVerticesAsArrayList());
					}
