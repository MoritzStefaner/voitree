...
import wblut.hemesh.core.HE_Intersection;
...
public class HEC_ShrinkWrap extends HEC_Creator {
	 ...
	 */
	public HEC_ShrinkWrap(final HE_Mesh mesh, final int level) {
		this();
		source = mesh;
		this.level = level;
	}
	...
	public HE_Mesh createBase() {
		...
		HE_Vertex v, vmod;
		WB_Ray R;
		final WB_AABBTree tree = new WB_AABBTree(source, 4);
		...
		while (undecided.size() > 0) {
			...
			for (int i = 0; i < undecided.size(); i++) {
				...
				if (lost) {
					...
					dist /= decNeighbors;
					final WB_Vector dv = v.subToVector(wcenter);
					dv.normalize();
