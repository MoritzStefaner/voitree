...
public class HEC_ConvexHull extends HEC_Creator {
	 ...
	 * @return self
	 */
	public HEC_ConvexHull setPoints(final Collection<WB_Point> points) {

		this.points = new WB_Point[points.size()];
		final Iterator<WB_Point> itr = points.iterator();
		int i = 0;
		while (itr.hasNext()) {
	...
	public HE_Mesh createBase() {
		...
		final WB_Point cog = new WB_Point();
	...
	private HE_Mesh addPointToHull(final HE_Mesh mesh, final int i) {
		...
		if (visibleFaces.numberOfFaces() > 0) {
			...
			final ArrayList<HE_Halfedge> halfedges = mesh
