...
import wblut.hemesh.modifiers.HEM_MultiSlice;
...
public class HEC_VoronoiSphere extends HEC_Creator {
	...
	public HE_Mesh createBase() {
		...
		if (approx) {
			...
			dir = new WB_Normal[numTracers];
			for (int i = 0; i < numTracers; i++) {
				dir[i] = new WB_Normal(tracers[i]);
				dir[i].normalize();
			...
			for (int j = 0; j < numberOfPoints; j++) {
				if (cellIndex != j) {
					...
					final WB_Normal N = new WB_Normal(points[cellIndex]);
					N.sub(points[j]);
	...
	private void grow(final WB_Point[] tracers, final int index) {
		final WB_KDTree<Integer> kdtree = new WB_KDTree<Integer>();
