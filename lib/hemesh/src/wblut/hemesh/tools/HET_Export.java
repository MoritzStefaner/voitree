...
public class HET_Export {
	...
	*/
	public static void saveToOBJ(final HE_Mesh mesh, final HET_OBJWriter obj) {
		...
		while (fItr.hasNext()) {
			f = fItr.next();
			obj.faceWithNormals(keyToIndex.get(f.getHalfedge().getVertex()
					.key())
					+ vOffset, keyToIndex.get(f.getHalfedge().getNextInFace()
					.getVertex().key())
					+ vOffset, keyToIndex.get(f.getHalfedge().getPrevInFace()
					.getVertex().key())
					+ vOffset, keyToIndex
					.get(f.getHalfedge().getVertex().key())
					+ nOffset, keyToIndex.get(f.getHalfedge().getNextInFace()
					.getVertex().key())
					+ nOffset, keyToIndex.get(f.getHalfedge().getPrevInFace()
					.getVertex().key())
					+ nOffset);
		}
	}
	...
	*/
	...
	public static void saveToSTL(final HE_Mesh mesh, final String path,
		...
		try {
			...
			while (fItr.hasNext()) {
				...
				final WB_Point v1 = f.getHalfedge().getVertex();
				final WB_Point v2 = f.getHalfedge().getNextInFace().getVertex();
	...
	public static void saveToHemesh(final HE_Mesh mesh, final String path) {
		...
		hem.sizes(mesh.numberOfVertices(), mesh.numberOfHalfedges(), mesh
				.numberOfEdges(), mesh.numberOfFaces());
