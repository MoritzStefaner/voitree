...
public class HE_Halfedge {
	...
	public WB_VertexType2D getHalfedgeType() {
		...
		WB_Vector v = _vertex.subToVector(getPrevInFace()._vertex);
		v.normalize();
		final WB_Vector vn = getNextInFace()._vertex.subToVector(_vertex);
		vn.normalize();
		v = v.cross(vn);
		final WB_Normal n;
		if (_face == null) {
	...
	public WB_Vector getHalfedgeTangent() {
		...
		if ((_pair != null) && (_vertex != null) && (_pair.getVertex() != null)) {
			final WB_Vector v = _pair.getVertex().subToVector(_vertex);
			v.normalize();
			return v;
		}
		return null;
	 ...
	 */
	public WB_Normal getHalfedgeNormal() {
		WB_Normal fn;
		...
		final WB_Normal _normal = new WB_Normal(vn);
