 ...
 * 
 */
public class HE_Vertex extends WB_Point {
	 ...
	 */
	public HE_Vertex(final WB_XYZ v) {
		super(v);
		_key = new Integer(_currentKey);
		_currentKey++;
		label = -1;
	 ...
	 * @return normal
	 */
	public WB_Normal getVertexNormal() {

		if (_halfedge == null) {
			return null;
		}
		HE_Halfedge he = _halfedge;
		final WB_Normal _normal = new WB_Normal();
		final ArrayList<WB_Normal> normals = new ArrayList<WB_Normal>();
		do {
			if (he.getFace() != null) {
				final WB_Normal fn = he.getFace().getFaceNormal();
				normals.add(fn);
			}
			he = he.getNextInVertex();
		} while (he != _halfedge);
		for (int i = 0; i < normals.size(); i++) {
			final WB_Normal ni = normals.get(i);
			boolean degenerate = false;
			for (int j = i + 1; j < normals.size(); j++) {
				final WB_Normal nj = normals.get(j);
				if (ni.isParallel(nj)) {
					degenerate = true;
					break;
				}
	...
	public WB_VertexType3D getVertexType() {
		...
		do {
			...
			final WB_Point v = he.getNextInFace().getVertex().get();
			v.sub(he.getVertex());
			he = he.getNextInVertex();
			...
			final WB_Normal c = f.getFaceNormal().cross(fn.getFaceNormal());
	 ...
	 * @return halfedges
	 */
	public ArrayList<HE_Halfedge> getHalfedgeStar() {
		final ArrayList<HE_Halfedge> vhe = new ArrayList<HE_Halfedge>();
		if (getHalfedge() == null) {
			return vhe;
		}
	 ...
	 * @return edges
	 */
	public ArrayList<HE_Edge> getEdgeStar() {

		final ArrayList<HE_Edge> ve = new ArrayList<HE_Edge>();
		if (getHalfedge() == null) {
			return ve;
		}
		HE_Halfedge he = getHalfedge();
	 ...
	 * @return faces
	 */
	public ArrayList<HE_Face> getFaceStar() {
		final ArrayList<HE_Face> vf = new ArrayList<HE_Face>();
		if (getHalfedge() == null) {
			return vf;
		}
		HE_Halfedge he = getHalfedge();
	 ...
	 * @return neighbors
	 */
	public ArrayList<HE_Vertex> getNeighborVertices() {
		final ArrayList<HE_Vertex> vv = new ArrayList<HE_Vertex>();
		if (getHalfedge() == null) {
			return vv;
		}
		HE_Halfedge he = getHalfedge();
