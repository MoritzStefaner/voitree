...
public class HEM_Soapfilm extends HEM_Modifier {
	...
	public HE_Mesh apply(final HE_Mesh mesh) {
		...
		for (int r = 0; r < iter; r++) {
			...
			final ArrayList<HE_Vertex> outer = sel.getOuterVertices();
			while (vItr.hasNext()) {
				v = vItr.next();
				if (outer.contains(v)) {
					newPositions[id] = v;
	...
	public HE_Mesh apply(final HE_Selection selection) {
		...
		selection.parent.triangulateFaces();
		WB_AABB box = new WB_AABB();
		if (autoRescale) {
	...
	private WB_Point minDirichletEnergy(final HE_Vertex v) {
		...
		final ArrayList<HE_Halfedge> hes = v.getHalfedgeStar();
		HE_Vertex neighbor;
		HE_Vertex corner;
		HE_Halfedge he;
		double cota;
		...
		for (int i = 0; i < hes.size(); i++) {
			...
			neighbor = he.getEndVertex();
			{
				...
				cota = WB_XYZ.cosAngleBetween(corner, neighbor, v);
				cotsum += cota / Math.sqrt(1 - cota * cota);
				corner = he.getPair().getPrevInFace().getVertex();
				cotb = WB_XYZ.cosAngleBetween(corner, neighbor, v);
