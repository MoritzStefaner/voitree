...
public class WB_Knot {
	 ...
	 */
	public WB_Knot(final int ncp, final int degree) {
	 ...
	 */
	public WB_Knot(final int ncp, final int degree, final double[] val) {
		this(ncp, degree);
	...
	public WB_Knot(final int degree, final double[] val) {
		m = val.length - 1;
		values = new double[m + 1];
		this.degree = degree;
		n = m - this.degree - 1;
	 ...
	 * @param knot
	 */
	public WB_Knot(final WB_Knot knot) {
		degree = knot.degree;
		n = knot.n;
		m = n + degree + 1;
		values = new double[m + 1];
	...
	public static WB_Knot merge(final WB_Knot UA, final WB_Knot UB) {
		if (UA.p() != UB.p()) {
			throw new IllegalArgumentException(
		...
		return new WB_Knot(UA.degree, allValues);
	}
}
