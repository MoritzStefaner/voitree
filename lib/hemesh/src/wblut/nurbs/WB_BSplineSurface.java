...
public class WB_BSplineSurface implements WB_Surface {
	protected WB_Knot		uknot;
	protected WB_Knot		vknot;
	...
	public WB_BSplineSurface(final WB_Point[][] controlPoints,
			final WB_Knot uknot, final WB_Knot vknot) {
	...
	public WB_BSplineSurface(final WB_Point[][] controlPoints,
		...
		uknot = new WB_Knot(controlPoints.length, udegree);
		vknot = new WB_Knot(controlPoints[0].length, vdegree);
		p = uknot.p();
		n = uknot.n();
		q = vknot.p();
		m = vknot.n();
	...
	public WB_BSplineSurface(final WB_Point point00, final WB_Point point10,
			final WB_Point point01, final WB_Point point11) {
		uknot = new WB_Knot(2, 1);
		vknot = new WB_Knot(2, 1);
		p = uknot.p();
		n = uknot.n();
		q = vknot.p();
		m = vknot.n();
	...
	public WB_BSplineSurface insertVKnot(final double v, final int r) {
		...
		final WB_Knot VQ = new WB_Knot(m + 1 + r, q);
		for (int i = 0; i <= k; i++) {
	...
	public WB_Knot uknot() {
		return uknot;
	}

	public WB_Knot vknot() {
		return vknot;
	}
	...
	public WB_BSplineSurface[] splitU(final double u) {
		...
		knot1.normalize();
		final WB_Point[][] points1 = new WB_Point[k + 1 - p][m + 1];
		for (int j = 0; j <= m; j++) {
			for (int i = 0; i < k + 1 - p; i++) {
				points1[i][j] = newBSplineSurface.points[i][j];
			}
		}
		final WB_Knot knot2 = new WB_Knot(km - k, p);
		for (int i = 0; i <= p; i++) {
			knot2.setValue(i, u);
		}
		...
		knot2.normalize();
		final WB_Point[][] points2 = new WB_Point[km - k][m + 1];
		for (int j = 0; j <= m; j++) {
	...
	public WB_BSplineSurface[] splitV(final double v) {
		...
		knot2.normalize();
		final WB_Point[][] points2 = new WB_Point[n + 1][km - k];
		for (int j = 0; j <= n; j++) {
			for (int i = 0; i < km - k; i++) {
